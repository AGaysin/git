...
#define SELECT()	GPIO_WriteLow(GPIOC, GPIO_PIN_3);	/* PB3: MMC CS = L */
#define	DESELECT()	GPIO_WriteHigh(GPIOC, GPIO_PIN_3);	/* PB3: MMC CS = H */
...
void read_blk_part(BYTE *buff, UINT ofs, UINT cnt)
{
  unsigned int i=0;
  FifoCt = 0;
  for (i=0;i<ofs; i++) rcv_spi();
  if (flag)
  {
    flag = 0;
    //fb_wave
    while(FifoCt<252) {Buff[FifoCt++]=rcv_spi();i++;}
    asm("NOP");
  }
  else
  {
    //fb_mem
  for (i=0;i<cnt; i++) *buff++ = rcv_spi();
  for (i=0; (i+cnt+ofs)<514; i++) rcv_spi();
  }
