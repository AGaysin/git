...
unsigned char Stream1[ARRAY_SIZE+1], Stream2[ARRAY_SIZE+1];
unsigned char PlayPtr, changeStream, interrupt;
          ...
          while(cnt) 
          {
            if (PlayPtr == 0 || PlayPtr == 2)
            {
              ...
              else
              {
                ...
                Stream1[str1write]=rcv_spi();
                if (Stream1[str1write] < 0x80) Stream1[str1write] += 0x80;
                else Stream1[str1write] -= 0x80;
                str1write++;
            ...
            else if (PlayPtr == 1)
            {
              ...
              else
              {
                //write to Stream 2
                rcv_spi();
                Stream2[str2write]=rcv_spi();
                if (Stream2[str2write] < 0x80) Stream2[str2write] += 0x80;
                else Stream2[str2write] -= 0x80;
                str2write++;
